...
![](pic/L1_array-to-map.png)

在建立樣板以後，我們就可以藉由關鍵的方法：「[`containsKey(Object key)`](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/HashMap.html#containsKey(java.lang.Object))」，以之來達到搜尋目標的目的；依照上圖的樣板，對於當前遍歷到的元素，我們只要查詢「Map」中是否存在一個「鍵」，其數值與當前元素的值之總和恰好為「`target`」（也就是查詢互補數 `target - nums[i]` 這個鍵是否存在），若存在，那麼這兩個元素就是題目所要求的答案，程式碼如下：
...
不過，這裡我們還可以再進一步優化：既然每次查詢的都是「互補數」，那不如在建立樣板時，就直接把「互補數（`target - nums[i]`）」當作「鍵」存入「Map」，如此一來，之後只要直接拿「當前元素的值」去查詢即可，省下每次查詢時再計算互補數的步驟，而這也就是本節「建議解答」所採用的寫法：
...
兩種寫法的核心邏輯完全相同，差別只在於「把互補數放在查詢時計算」還是「把互補數放在存入時計算」；前者對照上方的示意圖較好理解，後者則省去了重複計算，因此本文採用後者作為建議解答。
